...
exports.newInstance = function (i18n) {
    ...
    aweforms.fieldNg = _.bind(aweforms.fieldNg, aweforms);
...
function getField(context, $input, text) {
    ...
    var errors = modelValue && modelValue.errors;
    ...
    return {
        ...
        fieldValue: modelValue && modelValue[fieldName],
        error: error
    };
}
...
function alterInput(field, $input,forAngular) {
    ...
    if (forAngular) {
        $input.attr("ng-model",field.qualifiedId);
    } else {
        if ($input.is("select")) {
            if (fieldValue) {
...
function renderTemplateNg(context,modelField, self, templateText) {
    var error = modelField.modelName+".errors."+modelField.modelId;

    var label = self.i18n.__.call(context, modelField.qualifiedId);

    return [
        '<div class="field" ng-class="{\'text-error\':', error, '}">',
...
Aweforms.prototype.fieldNg = function () {
    var self = this;
    ...
    return function (text, render) {
        ...
        alterInput(field, $input,true);


        return renderTemplateNg(this,field, self, $);
    }
};